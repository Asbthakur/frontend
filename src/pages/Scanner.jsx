...
const Scanner = () => {
  ...
  // Check if user can scan - allow anonymous users (will be limited by backend/IP)
  ...
  const renderModeSelection = () => (
    <div className="max-w-2xl mx-auto">
      <div className="text-center mb-8">
        ...
        {/* Scan limit info - only show for logged in users */}
        {isAuthenticated && user && (
          <div className="mt-4 inline-flex items-center space-x-2 px-4 py-2 bg-primary-50 rounded-full">
            ...
            <span className="text-sm font-medium text-primary-700">
              {user.plan === 'pro'
                ? 'Unlimited scans'
                : `${user.dailyScansRemaining ?? 'Limited'} scans remaining today`}
            </span>
          </div>
        )}
      ...
      {isAuthenticated && user?.plan === 'free' && !canScan && (
