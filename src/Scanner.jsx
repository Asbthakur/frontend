...
const Scanner = () => {
  ...
  // Extract text from all images (with optional table detection)
  const extractTextFromAll = async (detectTables = false) => {
    ...
    try {
      ...
      if (detectTables && capturedImages.length === 1) {
        // Single image with table detection
...
${translatedText}
  ...
  const downloadAsPDF = async () => {
    ...
    try {
      ...
      if (response.success && response.pdf) {
        ...
        setError(response.error || 'Failed to create PDF');
      }
    } catch (err) {
  ...
  // Download detected tables as Excel
                      ...
                      {table.data?.slice(0, 10).map((row, rowIndex) => (
                        <tr key={rowIndex} className={rowIndex === 0 ? 'bg-blue-50 font-medium' : rowIndex % 2 === 0 ? 'bg-gray-50' : ''}>
                          {row?.map((cell, cellIndex) => (
                            <td key={cellIndex} className="px-3 py-2 border-b border-gray-100">
                              {cell || '-'}
                            </td>
                          ))}
                        </tr>
